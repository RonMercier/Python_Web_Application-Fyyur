...
def venues():
    ...
    try:
        ...
        flash(
            f"Sorry, unable to display the venues page.", category="error")
        abort(500)
    finally:
...
def delete_venue(venue_id):
    ...
    # clicking that button deletes the data from the db then redirects the user to the homepage.
    finally:
        db.session.close()
...
def artists():
    ...
    try:
        ...
        flash(
            f"Sorry, this page cannot be displayed!.", category="error")
        abort(500)
    finally:
...
def create_artist_submission():
    ...
    if form.validate():
        try:
            ...
            flash(
                f"Sorry, the artist could not be listed{' the phone number provided already exists in the database' if phoneExists else ''}!", category="error")
            db.session.rollback()
            abort(500)
        finally:
            db.session.close()
...
def shows():
    ...
    try:
        ...
        flash(
            f"Sorry, unable to display the shows page.", category="error")
        abort(500)
    finally:
